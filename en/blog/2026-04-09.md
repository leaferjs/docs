...
In our daily work, we have long relied on various online design tools to create posters, event visuals, and marketing materials. These tools are mature in functionality, but as usage deepened, several issues gradually became apparent:这些工具功能成熟，但随着使用深入，一些问题也逐渐显现出来：
...
Gzm Design 就是在这个背景下诞生的。Gzm Design was born in this context. It is an open-source online design tool built with Vue3 and LeaferJS, supporting PSD import, drag-and-drop editing, and AI image composition capabilities.
...
We previously evaluated several engines, including Fabric.js. After trying multiple options, we ultimately chose LeaferJS as the underlying rendering engine. The core reason is that it achieves a relatively balanced state across several key dimensions.做这个选择的核心原因，在于它在几个关键维度上达到了一个相对平衡的状态。
