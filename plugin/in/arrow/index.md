...
```html [arrow.min]
<script src="https://unpkg.com/@leafer-in/arrow@2.0.5/dist/arrow.min.js"></script>
<script>
...
```html [arrow]
<script src="https://unpkg.com/@leafer-in/arrow@2.0.5/dist/arrow.js"></script>
<script>
  const { Arrow } = LeaferIN.arrow
</script>
```
...
interface IPathDataArrow {
  ...
  fill?: boolean // v2.0.5 开始支持启用fill箭头，fill箭头的stroke不能为透明颜色值，可以整体设置元素的opacity实现
}
